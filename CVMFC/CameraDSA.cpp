...
IplImage* CCameraDS::QueryFrame()
{
	...
	m_pSampleGrabber->GetCurrentBuffer(&size, NULL);//第二个参数为NULL表示获得单帧图像药费的空间大小

	//if the buffer size changed
	if (size != m_nBufferSize)
	{
		if (m_pFrame)//如果开始已经捕获单帧图像存入，先释放资源，从新分配内存空间
			cvReleaseImage(&m_pFrame);
...
int CCameraDS::CameraCount()
{
   ...
   // enumerate all video capture devices
	CComPtr<ICreateDevEnum> pCreateDevEnum;
	//ICreateDevEnum *pCreateDevEnum;//ms
    HRESULT hr = CoCreateInstance(CLSID_SystemDeviceEnum, NULL, CLSCTX_INPROC_SERVER,
									IID_ICreateDevEnum, (void**)&pCreateDevEnum);
