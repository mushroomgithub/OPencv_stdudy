...
int  imageClone(IplImage* pi,IplImage** ppo)  //  复制 IplImage 位图pi给ppo
{
	if (*ppo) {
...
{                                           //  图像整数倍缩小
	...
	bpl=pi->widthStep;  //获得输入图像每行像素的字节数
	bplt=po->widthStep;//获得输出图像每行像素的字节数
	buf=pi->imageData;   //获得输入图像数据区首地址
	buft=po->imageData;//获得输出图像数据区首地址
	for (i=0;i<pi->height;i+=n) {//加n为了保证图像整数倍缩小
		for (j=0,k=0;j<pi->width*nCnls;k+=nCnls,j+=n*nCnls)
			memcpy(&buft[k],&buf[j],nCnls);   //  复制一个像素
		buf+=n*bpl;     buft+=bplt;//移动行
	}
